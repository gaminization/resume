...
This repository contains the flagship production LaTeX source (`resume.tex`), compiled single-page PDF (`resume.pdf`), official evidence attestation mapping (`Evidence_Mapping.md`), and comprehensive engineering report (`Resume_Optimization_Report.md`) for **Garv Arora**, engineered from the immutable **90% score baseline** (`garv_arora_resume_90pct_FINAL.tex`) and optimized for high interview conversion across **Software Engineering (SWE), Robotics, Autonomous Systems, Embedded Systems, Computer Vision, ADAS, and AI/ML Engineering**.

---

## Deliverable Files

- **`resume.tex`**: Production LaTeX source code built as a surgical patch onto the immutable 90% historical baseline foundation with 100% unique action verbs, full month experience dates, stacked SEDS India promotion, and plaintext hyperref configuration.
...
- **`CHANGELOG.md`**: Complete Change Budget, regression audit, and patch history built on top of the 90% immutable baseline.
